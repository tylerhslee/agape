...
mod tests {
    ...
    fn color() {
        ...
        rej("tool search(text q) -> text; sync text find(text q) { return search(q); }", ErrorClass::Color);
    ...
    fn authority() {
        ...
        ok("tool s(text q) -> text; agent R grants { use s } { text h = s(\"q\"); } spawn R r; awake r;");
        rej("tool s(text q) -> text; agent R { text h = s(\"q\"); } spawn R r; awake r;", ErrorClass::Authority);
        rej("tool s(text q) -> text; agent B grants { } {} agent C grants { use s } { extend B(); } spawn C c; awake c;", ErrorClass::Authority);
    }
    ...
    fn taint() {
        ...
        rej("authority Al; event Al(text m); tool fetch(text u) -> text; agent M grants { use fetch, emit Al } { text b = fetch(\"u\"); emit Al(b); } spawn M m; awake m;", ErrorClass::Taint);
