...
> (SPEC §15.5), the runtime contract (`RUNTIME_SPEC.md`), thesis #8 ("swap the config").
...
`<cli> serve` exposes. `RUNTIME_SPEC.md` is the cross-runtime contract; this
interface is the transport/API shape the toolchain exposes to frontends, and it
should be pinned **before** the two efforts drift:
