...
Target spec: `../SPEC.md` `v1.0.0-beta.2026.7.14.1` — the core kernel.
...
- Language suite: `.ag` accept/reject tests under `tests/` — currently `216` tests (`137` accept, `79` reject), including `06b_world` (the wired events/actions world interface), `06c_delegation` (the §6c task-send), and the §13 attestation protocol (pending principal decision, attester-match verification).
