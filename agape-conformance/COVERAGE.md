...
Target spec: `../SPEC.md` `v1.0.0-alpha.2026.7.2.2` — the core kernel.
...
- Language suite: `.ag` accept/reject tests under `tests/` — currently `164` tests (`101` accept, `63` reject).
- Runtime suite: TypeScript adapter tests under `../agape-runtime-conformance`.
- Core kernel only: the suite exercises the core kernel with no syntactic sugar. Gate branching is an ordinary `if` over `.committed` (no arm block); fusion is `quorum` with `independent`/`dependent` (no `all`/`any`/pipe); memory is store/recall over the `mem` handle plus the objective `select … from ledger` query (no `find`/`match`); rules carry inline `floor`/`readiness` (no `policy` declaration). The deferred library layer (modules, visibility, generics, interfaces), `retry`, and §20 ergonomics (reversible sinks) are not exercised.
