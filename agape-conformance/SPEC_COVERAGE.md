...
Target: `SPEC.md` `v1.0.0-beta.2026.8.6.0` — the core kernel.
...
| 10 Memory | covered | Structural named-clause descriptors with order-independent clauses; closed modality/scope/retention vocabularies; persistable payload types; exact store typing and deeply raw `TYPE[]` recall typing; no implicit cognition or write; no magic handle names; placement, legacy, and `mem<T>` diagnostics; descriptor reuse after forget; taint; the objective ledger query; and driver-retention configuration boundaries. Exact recalled values, empty misses, authenticated user isolation, generations, equal-value episodic origins, ranking/caps, durable restart, transactional reconciliation, and replay are `runtime-suite`. |
...
| 16.7 Memory runtime | runtime-suite | Compiler tests cover qualified descriptors, exact store typing, typed deep-tainted recall, structural placement, vocabulary, handle reuse after forget, capability config, and the missing-user crash boundary. Runtime/production tests for exact envelopes and misses, authenticated scope resolution and driver non-access on failure, generations, equal-value episodic origins, ranking/caps, durable restart, idempotent reconciliation, truthful receipts, provenance, and replay without live mutation are required and pending implementation. |
